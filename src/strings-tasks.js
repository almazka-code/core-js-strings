...
function removeLastOccurrences(str, value) {
  ...
  return a >= 0 ? str.slice(0, a) + str.slice(a + value.length) : str;
}
...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((acc, x, index) => acc + str.charCodeAt(index), 0)
    : 0;
}
...
function countVowels(str) {
  const regexp = str.match(/[aeiouy]/gi);
...
function isPalindrome(str) {
  const newString = str.toLowerCase().replace(/[\W_]/g, '');
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (x) => newABC[abc.indexOf(x)]);
}
